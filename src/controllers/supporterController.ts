...
export const supporterController = {
  ...
  getPendingTickets: async (req: Request, res: Response) => {
    try {
      ...
      const tickets = await supporterService.getPendingTicketsByTeam(supporter.team_id);
  ...
  getRespondedTickets: async (req: Request, res: Response) => {
    try {
      const { supporter_id } = req.params;
      ...
      // Get responded tickets for the supporter's team
      const tickets = await supporterService.getRespondedTicketsByTeam(supporter.team_id);
      res.status(200).json(tickets);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
